...
cd $QUDA_BUILD
cmake -DCMAKE_BUILD_TYPE=DEVEL -DQUDA_DIRAC_DEFAULT_OFF=ON -DQUDA_DIRAC_STAGGERED=ON -DQUDA_GAUGE_ALG=ON -DQUDA_GAUGE_TOOLS=ON -DQUDA_DOWNLOAD_USQCD=ON -DQUDA_QIO=ON -DQUDA_QMP=ON -DQUDA_MULTIGRID=ON -DQUDA_MULTIGRID_NVEC_LIST="6,24,48,64,96" -DQUDA_TARGET_TYPE="HIP" -DROCM_PATH=${ROCM_PATH} -DGPU_TARGETS="gfx90a" -DCMAKE_CXX_COMPILER="hipcc" -DCMAKE_C_COMPILER="hipcc" -DCMAKE_CXX_FLAGS="${MPI_CFLAGS}" -DCMAKE_C_FLAGS="${MPI_CFLAGS}" -DCMAKE_EXE_LINKER_FLAGS="${MPI_LDFLAGS}" -DCMAKE_SHARED_LINKER_FLAGS="${MPI_LDFLAGS}" -DCMAKE_C_STANDARD=99 -DCMAKE_INSTALL_PREFIX=${INSTALLROOT}/quda $QUDA_SRC
